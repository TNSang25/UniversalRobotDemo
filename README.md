...
### 1. `ur_description`
Chứa toàn bộ các file cấu hình về URDF, Xacro, Mesh, và các thông số vật lý của các dòng tay máy UR khác nhau (ur3, ur3e, ur5, ur5e, ur10, ur10e...). 
...
### 2. `ur_simulation_gz`
Mô phỏng tay máy UR tích hợp với Gazebo (phiên bản mới) và MoveIt 2. 
...
Chứa các node (viết bằng C++) tương tác với `MoveGroupInterface` của MoveIt 2 để điều khiển đầu cuối của tay máy (end-effector: `tool0`) di chuyển theo một quỹ đạo đặc biệt trong không gian.
...
```
*Kết quả:* Đường viền Marker màu đỏ dạng chữ S xuất hiện trên RViz và robot sẽ mượt mà vẽ theo quỹ đạo hình chữ S đó.
